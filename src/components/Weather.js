        ...
        overview.map((data, i) => (
          <div className="box bg-low-navy p-7 w-full md:w-52 lg:w-56 mr-5 mb-5 flex justify-center items-center rounded-lg" key={i}>
            ...
            <div className="text pl-4">
              <p className="text-light-gray font-light text-xs lg:text-sm capitalize">{data.text}</p>
              { data && <h3 className="text-white font-semibold text-xl lg:text-2xl">{data.description}{data.behind}</h3>}
            </div>
          </div>
        ))
      }
